...
#' @param pDeteced
#' @param config.xcms
signalAlignmentObiwarp <- function(pDeteced,
                                   config.xcms, 
                                   binSize = 0.05,
                                   is.drake = FALSE,
                                   BPPARAM = bpparam()) {
        ...
        if (is.drake) {
                ...
                if (phase != names(config.xcms)) {
                        stop("     x Names of pDeted and config.xcms are different")
                }
                pDeteced    <- pDeteced[[1]]
                config.xcms <- config.xcms[[1]]
        }
        ...
        } else if (pDeteced$state == 1) {
                obiwarp <- list(
                        method = "obiwarp",
                        xdataTmAdj = NULL,
                        err.msg = "Some errors occured in previous steps",
                        state = 1
                )
        } else {
                stop("     x Unknown error")
        }
...
#' @param pDeteced
#' @param config.xcms
signalAlignmentPeakGrp <- function(pDeteced, 
                                   config.xcms, 
                                   peakDensity.minSamples = 3, 
                                   peakDensity.minfrac = 0.8, 
        ...
        if (pDeteced$state == 0) {
                ...
                tryCatch({        
                        ...
                        pdPar <- PeakDensityParam(
                                # 2-1-1 setting peak density parameters -----------------------------------------
                                # bw: the bandwidth (standard deviation ot the 
                                #       smoothing kernel) to be used
                                bw = ifelse(
                                        test = is.na(config.xcms$bw), 
                                        yes = 20,
                                        no = config.xcms$bw
                                ),
                                # minFraction: the minimum fraction of samples 
                                #       in at least one sample group in which
                                #       the peaks have to be present to be
                                #       considered as a peak group
                                minFraction = peakDensity.minfrac,
                                # minSamples: the minimum number of samples in 
                                #       at least one sample group in which the 
                                #       peaks have to be detected to be considered 
                                #       a peak group
                                minSamples  = min(
                                        min(table(smpGrp)), 
                                            peakDensity.minSamples
                                ),
                                # sampleGroups: A vector of the same length than 
                                #       samples defining the sample group assignments
                                sampleGroups = smpGrp
                        )

                        # 2-1-2 group correspondence peaks
                        xdataGrpCrmP <- groupChromPeaks(
                                object = xdata,
                                param = pdPar
                        )
...
#' @param xdataTmAdj
#' @param config.xcms
visualizingAlignmentRst <- function(xdataTmAdj, config.xcms, is.drake = FALSE) {
        
        if (is.drake) {
                phase      <- names(xdataTmAdj)
                if (phase != names(config.xcms)) {
                        stop("     x Phases of xData and config are different.")
                }
                xdataTmAdj  <- xdataTmAdj[[1]]
                config.xcms <- config.xcms[[1]]
        }

        if (xdataTmAdj$state == 0) {
                rtAdjmethod <- xdataTmAdj$method
                xdataTmAdj  <- xdataTmAdj$xdataTmAdj
                xdataTmAdjPeaks <- as.data.table(
                        chromPeaks(xdataTmAdj), 
                        keep.rownames = "peakId"
                )
